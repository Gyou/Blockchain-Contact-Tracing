    ...
    while (True):
        ...
        for i in range(0, len(Users)):
            ...
            for j in range(0, len_contact):
                # randomly choose one as a contact
                dev_id = random.randint(0, len(Users) - 1)
                # to avoid contact with itself
                while (dev_id == i or (dev_id in contact_list)):
                    dev_id = random.randint(0, len(Users) - 1)
            ...
            for j in range(0, len_witness):
                ...
                dev_id = i
                while (dev_id == i or (dev_id in user.cont_rec[timestamp - 1]) or (dev_id in wit_list)):
                    dev_id = random.randint(0, len(Users) - 1)
...
def draw_decentralized_figure(Users,Blockchain_Storage):
        ...
        if len(Blockchain_Storage) == record_point:
            plt.savefig("new"+str(User_num)+'diff_user_50_ban_reward_non_restricted_BenchMark_'+str(record_point)+'.pdf')
        plt.show()

        Gini_Change = [[],[],[]]
        Gini_Change_X_Stick = []
        with open('Gini_record2.txt','r') as f:
            r = f.readline()
            while(r):
                line = r.split(" ")
                #print(line)
...
def draw_gini_curve(Gini_Change,Gini_Change_X_Stick,Blockchain_Storage,Users):
    ...
    if len(Blockchain_Storage) == record_point:
        plt.savefig("new"+str(User_num)+'diff_user_50_ban_reward_non_restricted_BenchMark_Gini_'+str(record_point)+'.pdf')
    plt.show()
...
if __name__ == '__main__':
    ...
    if os.path.exists('Gini_record2.txt'):
        os.remove("Gini_record2.txt")
