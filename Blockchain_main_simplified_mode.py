# this python file simulates the blockchain framework designed During July and Aug. 2021.

# this file is only for test how the distribution of user credit and user balance will be, we assume all trasanctions are verified
# here, the public key is actually the dev_id
import threading
import os, time
import math, random
import uuid
...
gen_tran_cred_reward = 1
sign_tran_cred_reward = 1
mine_block_cred_reward = 1
mine_block_mon_reward = 5  # this varies for different user
failure_cred_punish = 5
...
class User:
    def __init__(self, id, credit, balance, mean_cont, var_cont, mean_wit,var_wit):
        ...
        self.u_w = mean_wit
        self.d_w = var_wit
        self.mined_count = 0.00001 # simulate a small number, not 0
            ...
            if (len(self.cont_rec) >= (self.old_len + gen_freq)):
                ...
                for i in range(self.old_len, self.old_len + gen_freq):
                    ...
                    for dev_id in self.cont_rec[i]:
                        ...
                        Users[dev_id].credit += sign_tran_cred_reward
                ...
                if flag ==1:
                    ...
                    self.credit += gen_tran_cred_reward
...
def vote_for_new_Candidate_Miners():
    ...
    for user in Users:
        ...
        #voted_user_index = ratio_based_vote(cum_crdt_ratio_list)
    ...
    for i in range(len(vote_record)):
        ...
        total_score[i] = total_score[i]*((math.pow(Users[i].credit / np.sum(user_credit),0.2) +1)/2)
         #* math.exp((np.sum(user_mined_count)+1)/(Users[i].mined_count+1) ) #*(np.sum(user_mined_count)+1)/(Users[i].mined_count+1) ##(1 - Users[i].mined_count / np.sum(user_mined_count))
    ...
    print("candidate Miners",selected_index)
...
def blockchain_mining():
    ...
    while (True):
        ...
        while(random.uniform(0,1)<=0.001):
            miner.credit -= failure_cred_punish
        ...
        #print('mined block',"miner:",chosen_one)
        ...
        corrected_reward = mine_block_mon_reward * math.pow(1 - (len(miner.tran_msg) + 1) / (total_tran + 1), 1.5)
        print("user", miner.dev_id, "get balance reward:", corrected_reward)
        miner.balance += corrected_reward
...
def capute_gini_list(credit,balance, mining_count):
    gini_list = [round(qe.gini_coefficient(credit), 2), round(qe.gini_coefficient(balance), 2),
                 round(qe.gini_coefficient(mining_count), 2)]
    return gini_list

def draw_decentralized_figure(Users,Blockchain_Storage):
        #print(len(Blockchain_Storage))
        # if len(Blockchain_Storage)%5 != 0:
        #     return
        credit = []
        balance = []
        mining_count = []
        ...
        #balance.sort()
        #mining_count.sort()
        # getting the add on credits. balance
        ...
        gini_list = capute_gini_list(credit,balance,mining_count)
        fo = open("Gini_record.txt", "a")
        ...
        for i in range(len(name_list)):
            ...
            ax.plot(f_vals*100, l_vals*100, label=name_list[i] + ', gini = ' + str(gini_list[i]))
        ax.plot(f_vals*100, f_vals*100, label='equality, gini = 0')
        ax.legend()

        plt.title('Total user '+str(len(Users))+', Mined '+ str(len(Blockchain_Storage)) + ' Blocks')
        ...
        if len(Blockchain_Storage) in [1000,3000, record_point] :
            plt.savefig( "new"+str(User_num)+'diff_user_5_ban_reward_non_restricted_'+str(len(Blockchain_Storage))+'.pdf')
        plt.show()
        #print('before draw')
        Gini_Change = [[],[],[]]
        Gini_Change_X_Stick = []
        with open('Gini_record.txt','r') as f:
            r = f.readline()
            while(r):
                line = r.split(" ")
                #print(line)
...
def draw_gini_curve(Gini_Change,Gini_Change_X_Stick,Blockchain_Storage,Users):
    ...
    plt.title('Total user ' + str(len(Users)) + ', Gini Index with Height of block')
    if len(Blockchain_Storage) in [1000,3000, record_point] :
        plt.savefig("new"+str(User_num)+'diff_user_5_ban_reward_non_restricted_Gini'+str(len(Blockchain_Storage))+'.pdf')
    plt.show()
...
if __name__ == '__main__':
    ...
    if os.path.exists('Gini_record.txt'):
        os.remove("Gini_record.txt")
